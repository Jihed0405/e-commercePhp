...
function doInsert(){
	...
	function doEdit(){
  	...
  	// 		$res = $customer->single_customer($_GET['customerid']); 
  			...
  			$query = "SELECT * FROM `summary` s ,`customer` c 
				WHERE   s.`CUSTOMERID`=c.`CUSTOMERID` and ORDEREDNUM=".$_GET['id'];
			$mydb->setQuery($query);
			$cur = $mydb->loadSingleResult();
 

	        $sql = "INSERT INTO `messageout` (`Id`, `MessageTo`, `MessageFrom`, `MessageText`) VALUES (Null, '".$cur->PHONE."','Janno','FROM Jihed Ben Othmen : Your order has been '".$status. "'. The amount is '".$cur->PAYMENT. "')";
	        $mydb->setQuery($sql);
	        $mydb->executeQuery();
			...
			$query = "SELECT * 
				FROM  `product` p,`order` o,  `summary` s
			...
			foreach ($cur as $result) {
	  		 ...
	  		 $sql = "INSERT INTO `messageout` (`Id`, `MessageTo`, `MessageFrom`, `MessageText`) VALUES (Null, '".$result->OWNERPHONE."','Janno','FROM Jihed Ben Othmen : Your  product has been ordered'. The amount is '".$result->PAYMENT. "')";
	        $mydb->setQuery($sql);
	        $mydb->executeQuery();
