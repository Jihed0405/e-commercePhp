...
// FROM  `customer` c,  `summary` s, product p, order o
// WHERE p.`PROID` = o.`PROID` 
// AND s.`ORDEREDNUM` = o.`ORDEREDNUM` 
// AND c.`CUSTOMERID` = s.`CUSTOMERID` 
	$query = "SELECT  *  
				FROM  `customer` c,  `summary` s WHERE  c.`CUSTOMERID` = s.`CUSTOMERID` AND  ORDEREDSTATS='Confirmed' AND date(ORDEREDDATE)>='".$_POST['start']."' AND date(ORDEREDDATE) <='".$_POST['end']."'";
