...
// $query = "SELECT  *  FROM  `customer` c,  `summary` s WHERE  c.`CUSTOMERID` = s.`CUSTOMERID` AND  ORDEREDSTATS='Confirmed' AND date(ORDEREDDATE)>='". date_format(date_create($_POST['date_pickerfrom']), "Y-m-d")."' AND date(ORDEREDDATE) <='". date_format(date_create($_POST['date_pickerto']), "Y-m-d")."'";
// $query="SELECT *,SUM(ORDEREDQTY) as 'QTY'  FROM `product` P  ,`promopro` PR ,`order` O, `summary` S ,`customer` C 
...
// $query = "SELECT  *  FROM  `customer` c,  `summary` s 
