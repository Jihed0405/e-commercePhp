<h3>Wish List</h3>  
             <form   method="POST" action="<?php echo web_root ; ?>cart/controller.php?action=add">           
                    ...
                    <tbody>
                        <?php 
                            $query = "SELECT * FROM `product` P , `wishlist` W , `customer`  C , `promopro` PR, `category` CT  WHERE P.`PROID`=W.`PROID` AND W.`CUSID`=C.`CUSTOMERID`  AND P.`PROID`=PR.`PROID` AND P.`CATEGID`=CT.`CATEGID` AND WISHSTATS=0 AND  `CUSTOMERID`=".$_SESSION['CUSID'] ." ORDER BY `WISHDATE` desc ";
                          $mydb->setQuery($query);
